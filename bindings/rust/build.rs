fn main() {
    ...
    if !"queries/snakemake/highlights.scm".is_empty() && std::path::Path::new("queries/snakemake/highlights.scm").exists() {
        println!("cargo:rustc-cfg=with_highlights_query");
    }
    println!("cargo:rustc-check-cfg=cfg(with_injections_query)");
    if !"queries/snakemake/injections.scm".is_empty() && std::path::Path::new("queries/snakemake/injections.scm").exists() {
        println!("cargo:rustc-cfg=with_injections_query");
    }
    println!("cargo:rustc-check-cfg=cfg(with_locals_query)");
    if !"queries/snakemake/locals.scm".is_empty() && std::path::Path::new("queries/snakemake/locals.scm").exists() {
