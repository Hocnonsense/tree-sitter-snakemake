...
Run `pixi run codegen` to generate files (`src/parser.c`, `bindings/{python,rust,go}/*`, `Cargo.toml`, etc.) for binding to other languages.
...
Run `pixi run test` to quickly test if changes work, and `pixi run test-fresh` to regenerate from scratch and test.

To update the `tree-sitter-python` dependency:
```bash
pixi run --environment codegen npm install tree-sitter-python@latest
# or update al dependencies
pixi run --environment codegen npm install
```
